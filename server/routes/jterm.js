...
async function ensureJtermAtlas() {
  const existing = await db.getAtlasByCode(JTERM_CODE);
  if (existing) return existing;

  await db.findOrCreateUser({
    id: OWNER_ID,
    email: 'jterm@friendatlas.local',
    name: OWNER_NAME,
    profilePicture: null,
  });
...
async function geocodeLocation(query) {
  ...
  if (typeof fetch === 'function') {
    try {
      ...
      console.warn('J-Term geocode failed:', error.message);
    }
  }
...
router.post('/join',
  ...
  async (req, res) => {
    try {
      ...
      console.error('J-Term join error:', error);
      res.status(error.statusCode || 500).json({ error: error.message || 'Failed to join the J-Term Atlas' });
    }
  }
);
